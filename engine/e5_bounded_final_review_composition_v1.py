...
def compose_e5_bounded_final_review_v1(
    *,
    ...
    try:
        ...
        if allowed_route:
            _require(claude_counts_are_ints)
            measured = claude_measured_input_tokens
            requested = claude_requested_output_tokens
        else:
            _require(claude_counts_are_none)
            measured = 0
            requested = 0
        _require(type(measured) is int)
        _require(type(requested) is int)
